...
class DataManager {
    ...
    async initData() {
        ...
        try {
            if (typeof initSupabase !== 'undefined') {
                await initSupabase();
                this.useSupabase = isSupabaseReady();
                console.log('Supabase ready:', this.useSupabase);
            }
        } catch (error) {
            console.warn('Supabase initialization failed, using localStorage:', error.message);
            this.useSupabase = false;
        }

        // Initialize default localStorage data if needed
    ...
    async getProducts() {
        try {
            if (this.useSupabase && typeof getProducts === 'function') {
                return await getProducts();
            }
        } catch (error) {
            console.warn('Supabase getProducts failed, using localStorage:', error.message);
        }
    ...
    async updateProduct(id, updatedProduct) {
        try {
            if (this.useSupabase && typeof updateProduct === 'function') {
                ...
                console.log('✓ Product updated in Supabase and localStorage');
                return result;
            }
        } catch (error) {
    ...
    async deleteProduct(id) {
        try {
            if (this.useSupabase && typeof deleteProduct === 'function') {
                await deleteProduct(id);
                // Sync to localStorage
                let products = this.getData('products') || [];
                products = products.filter(p => p.id !== id);
                this.setData('products', products);
                console.log('✓ Product deleted from Supabase and localStorage');
                return;
            }
        } catch (error) {
    ...
    async getSales() {
        try {
            if (this.useSupabase && typeof getSales === 'function') {
                return await getSales();
            }
        } catch (error) {
            console.warn('Supabase getSales failed, using localStorage:', error.message);
        }
    ...
    async addSale(sale) {
        try {
            if (this.useSupabase && typeof createSale === 'function') {
                const result = await createSale(sale);
                // Sync to localStorage
                const sales = this.getData('sales') || [];
                sales.push(result);
                this.setData('sales', sales);
                console.log('✓ Sale added to Supabase and localStorage');
                return result;
            }
        } catch (error) {
            console.warn('Supabase addSale failed, using localStorage:', error.message);
        }

        // Fallback to localStorage
        const sales = this.getData('sales') || [];
        sale.id = Math.max(...sales.map(s => s.id || 0), 0) + 1;
    ...
    async getDashboardStats() {
        ...
        return {
            ...
            totalProfit,
            totalLoss,
            categoryWiseSummary: this.getCategoryWiseSummary(products),
            lowStockProducts: this.getLowStockProducts()
        };
    }

    // Get category-wise summary
    getCategoryWiseSummary(products) {
        const categories = {};
    ...
    async getProfitLossReport(filterDate = null) {
        const sales = await this.getSales();
